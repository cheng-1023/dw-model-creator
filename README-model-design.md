# dw-model-creator（模型设计版）

> 本文档由 `README.md` 拷贝调整而来：Skill 边界缩减至**数仓模型设计**，原应用实体设计相关能力移出范围。`README.md` 原文保持不变，作为完整边界的上游参考。

`dw-model-creator` 是面向银行数据仓库场景的建模 Skill 项目，用于把数仓模型、指标、标签等模型设计需求，转换为可递归求解、可追溯、可校验的数仓模型设计。

项目采用“反向定义、正向验证”的工作方式：先从模型需求的字段和粒度出发，按需求端目标层模型 → 上游层（DM → DWM → DWD）识别能力缺口，再从 DWD → DWM → DM 验证模型、字段和依赖链。每个数仓模型必须依次完成概念模型、逻辑模型和物理模型设计；物理模型确认后，才允许按目标引擎生成 DDL，默认引擎为 PostgreSQL。

## 边界说明

### 范围内

- 输入：显式 DWD、DWM、DM 模型需求，以及驱动模型设计的指标、标签口径需求。
- 处理：概念模型 → 逻辑模型 → 物理模型三阶段设计；粒度、维度、度量、键与约束设计；分层规则与依赖矩阵校验；资产复用判定；人工介入回流。
- 输出：三阶段模型设计 Markdown 交付物；物理模型确认后的目标引擎 DDL（默认 PostgreSQL）。

### 范围外

- 报表、离线文件、API、推送等应用实体的应用契约设计（布局、筛选、分页、触发与推送规则等）。
- 应用端字段绑定与应用端使用级验证链（应用字段 → DM → DWM → DWD）。
- 指标、标签的应用发布、推送和 API 暴露等交付治理。
- 生产资产修改与生产作业实施。
...
| `assets/policies/` | 保存只读版本化的依赖矩阵、命名、主题和分层规则 CSV Bundle；Markdown 只解释规则，具有 manifest 和 checksum 的 CSV 是执行事实源。 |
