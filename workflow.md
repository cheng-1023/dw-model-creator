# dw-model-creator 核心工作流

> 文档状态：核心流程已讨论确认，待纳入 Skill 实现
> 适用阶段：一期
> 更新日期：2026-08-17
...
本工作流用于把一个银行数仓需求拆解为可递归求解、可追溯、可校验的需求实体和最小能力单元，并形成应用契约、语义定义、DM/DWM/DWD 模型设计及目标引擎 DDL；未指定引擎时默认使用 PostgreSQL。

一期约束：

- 一个 Request 可以包含报表、离线文件、API、推送、指标、标签和物理模型等多个 RequirementEntity。
- 每个 RequirementEntity 拆分为多个 CapabilityNeed；CapabilityNeed 是递归队列中的最小处理单元。
- 需求定义和缺口分析按应用端向上游反向展开：应用端 → DM → DWM → DWD。
- 正向过程只负责从 DWD → DWM → DM → 应用端验证可实现性、字段绑定和影响，不重新定义应用需求。
...
1. **反向定义，正向验证**：先从应用端确定字段和粒度，再递归推导 DM、DWM、DWD 缺口；完成反向闭包后，再从 DWD 向应用端验证。
2. **交付范围不等于分析范围**：用户可以只要求报表设计，但内部仍需分析支撑该报表的语义和物理能力。
...
```mermaid
flowchart TD
    R["Request：一次完整业务需求"] --> E1["RequirementEntity：报表"]
    R --> E2["RequirementEntity：离线文件"]
    R --> E3["RequirementEntity：指标"]
    R --> EN["其他显式或派生实体"]
    E1 --> N1["报表专属 Need"]
    E1 --> NS["共享 CapabilityNeed"]
    E2 --> N2["文件专属 Need"]
    E2 --> NS
    E3 --> N3["指标 CapabilityNeed"]
...
| 类型代码 | 说明 |
|---|---|
| `report` | 面向用户展示、筛选、钻取和导出的报表应用 |
| `offline-file` | 以 CSV 交付的数据文件应用 |
| `api` | 通过接口查询或交换数据的应用 |
| `push` | 按事件或计划向目标接收方推送数据的应用 |
| `metric` | 具有独立口径、粒度、公式和版本的治理指标 |
| `tag` | 基于字段或指标规则识别目标对象的治理标签 |
| `physical-model` | 用户显式提出的 DWD、DWM 或 DM 数仓模型需求；仍须执行三阶段设计 |

一个实体既可以由用户显式提出，也可以在递归过程中派生。例如报表使用了尚未发布的指标时，可以派生指标实体。
...
“指标”作为独立治理资产时建立 metric RequirementEntity；报表中的对应字段 Need 引用该指标实体，不重复定义口径。

`type_payload` 至少覆盖以下差异：

| 实体类型 | 类型化契约重点 |
|---|---|
| report | 行列布局、筛选、钻取、排序和展示粒度 |
| offline-file | 文件格式固定为 CSV，并定义字段顺序、分隔/编码、交付频率和落地位置 |
| api | 请求参数、响应字段、分页、错误语义、时效和权限 |
| push | 触发条件、接收方、载荷、频率、重试和幂等 |
...
| `delivery-field` | 应用实体要求交付的字段、别名、顺序和展示约束 |
| `delivery-rule` | 应用实体的筛选、排序、格式、触发或文件交付规则 |
| `non-functional` | SLA、质量、安全、权限、保留和审计能力 |
...
| requirement | `capability_acceptance_criteria` | 不依赖具体应用实体的能力级验收条件 |
...
| resolution | `binding_path` | 根据 `bindings[]` 排序得到的应用端 → DM → DWM → DWD 可读视图，不作为唯一事实源 |
...
`entity_usages[]` 至少包含稳定 `usage_id`、`usage_fingerprint`、`entity_id`、`usage_role`、`requiredness`、`presentation_grain_ref`、输出别名/交付约束、实体级 `acceptance_criteria`、`binding_refs[]`、`evidence_refs[]`、`validation_result`、`blockers[]` 和独立 `usage_state`。`state` 与 `usage_state` 都显式引用同一个 ResolutionState 类型。同一实体在不同展示位置或粒度使用同一能力时保存多个 usage 项。`requiredness=alternative` 时还必须包含 `alternative_group_id` 和该组的 `min_satisfied`。这样既保持 Need 与 Binding 合并，又不会把报表、文件或 API 的不同使用语境混成一份全局状态。

Need 的 `state` 表示规范化能力本身是否在当前 scope 内解决；`usage_state` 表示该能力是否满足某个实体的展示粒度、SLA、安全和交付约束。二者必须分开：例如共享度量能力可以是 resolved，但 API 的低延迟 usage 仍可 awaiting-human，而报表 usage 保持 resolved。
...
| delivery-field / delivery-rule | 输出类型、映射/格式、排序、投影和交付约束 |
| non-functional | SLA、质量、安全、权限、保留和审计要求 |
...
多个实体可以共享同一个规范化 Need。例如报表和数据文件需要同口径、同粒度、同时间语义的月末贷款余额时，不应重复设计；它们各自的展示角色、必需性、展示粒度和验收条件保存在不同 `entity_usages[]` 项中。
...
模型负责把同义业务描述映射为 `need_semantic_key_parts.csv` 的结构化组成行；脚本只对字段顺序、行顺序、枚举、单位、精度和表达式均已规范化的 CSV 记录集合计算 canonical hash，不直接对自由文本做相等判断。原生粒度、时间语义、过滤、单位/币种、安全范围、数据类型/精度或 SLA 不同且会改变能力契约时不得去重。

展示名称、列顺序和文件格式不同不会自动产生新的语义 Need；这些差异保存在实体使用项，或在确实需要独立求解时形成 delivery-field / delivery-rule Need。
...
| `requirement_entities.csv` | RequirementEntity | 保存应用、指标、标签和物理模型等实体主记录 |
| `entity_specs_*.csv` | 实体类型载荷 | 按 report、offline-file、api、push、metric、tag、physical-model 拆分类型专属字段 |
...
| `need_payload_*.csv` | Need 类型载荷 | 按 grain、dimension、measure、metric、tag、delivery、non-functional 拆分类型专属字段 |
...
| `bindings.csv` | 分层绑定 | 保存应用端、DM、DWM、DWD 各跳提供方和转换 |
...
| `gate_results.csv` | 门禁结果 | 保存矩阵、粒度、循环、质量、安全、模型阶段和方言校验 |
...
例如原报表粒度为“月份×机构”，新增产品作为 grain-key 会改变行粒度；如果已有 product_key，只增加 product_name 展示字段，则通常属于兼容扩展。
...
| 逻辑模型 logical | 把概念模型转化为与引擎无关的数据结构 | 模型粒度、事实/维度角色、属性、度量、业务键、关系基数、时态、质量和安全规则；每个逻辑字段的类型与说明 | 引用的概念 fingerprint 未变化；粒度和键合法；字段类型/说明完整；状态 confirmed |
| 物理模型 physical | 把逻辑模型落到目标层和目标引擎 | 表、字段、物理类型、精度、空值、默认值、主外键、唯一约束、索引、分区、存储、字段映射及表/字段说明 | 引用的逻辑 fingerprint 未变化；分层处理单元完成；矩阵、命名、类型、质量、安全和方言校验通过；P-GATE 只推进至 validated/awaiting-confirmation，PhysicalModelConfirmation 才推进至 confirmed |
...
8. physical 阶段状态严格按 designing → validated → awaiting-confirmation → confirmed 推进；P-GATE、应用验证、人工确认分别完成对应转换，任何一步都不能直接跨到 confirmed。
...
- 目标引擎、引擎 profile（方言和物理类型映射的唯一事实源）、`handler_version` 或任一 `policy_snapshot_refs`（仅包括命名、主题、质量、安全和层级策略）变化：在同一 revision 内原子地把受影响 physical、binding、确认和 DDL 全部标记 stale，再执行对应分层处理单元；
...
| `policy_snapshot_refs` | 命名、主题、质量、安全和层级策略快照引用；不包含引擎物理类型映射 |
...
5. 主键、唯一性、外键/引用、分区、索引、质量、安全、命名和依赖矩阵门禁全部通过。
...
flowchart TD
    ...
    D --> E["反向语义规划：应用端 → DM → DWM → DWD 的概念/逻辑模型"]
    E --> F["正向物理设计：DWD → DWM → DM 专属处理单元"]
    F --> G["应用端候选验证并生成待确认 Markdown"]
...
1. 从用户需求识别显式实体，例如报表、文件和指标。
2. 根据应用字段和治理要求派生必要实体，例如尚未发布的指标或标签。
3. 对每个实体拆分粒度、维度键、展示字段、度量、指标、标签、交付规则和非功能 Need。
...
8. 当前层存在缺口时，按 Need 类型和依赖矩阵派生下一层 Need：
   - 报表、文件、API、推送等应用交付 Need：先补充 DM，再沿 DM → DWM → DWD 递归；
   - 原子 measure / metric：补充承载口径和原子字段的 DWM Need，再沿允许路径补充 DWD Need；需要应用发布或物化时，另在同一 Need 中建立到 DM 的合法绑定；
...
10. 反向语义闭包只把 Need 推进到 partial / semantic-ready；只有正向物理设计、精确版本确认和应用端验证完成后，能力 `state` 与对应 `usage_state` 才能变为 resolved。
...
### 5.5 阶段四：正向物理设计、确认和应用验证

不等待整个 Request 全局闭包。每轮先按根实体或独立依赖子图计算 `forward-ready` 集合；仅对 conceptual/logical 已闭包的子图正向推进。应用实体通常验证完整 DWD → DWM → DM → 应用端链；显式 physical-model 实体只推进到其 `target_layer`，但该目标模型自身仍必须完成三阶段：
...
4. 对该分支完整适用 candidate 链执行契约验证：应用实体验证应用字段、粒度、键、时态、指标/标签版本、筛选、质量、安全和 SLA；显式 physical-model 实体验证其目标层模型契约。全部适用层 P-GATE 和该验证均通过后，才生成待确认模型 Markdown。
...
7. 验证每个应用字段是否能沿候选 DM、DWM、DWD 物理字段追溯到 logical 字段和 CapabilityNeed。
...
为每个可被依赖节点计算 `dependency_projection_fingerprint`，至少包含契约、resolution 状态、资产决策、证据版本/可信度和矩阵判定。该投影相对已传播水位发生变化时，把直接父 Need 和下游应用标记 stale 并级联重检：
...
外层固定点不是独立于门禁的一次事后检查，而是完整迭代“反向语义规划与父级回卷 → 三阶段模型门禁 → 正向分层物理设计与确认 → 应用端验证 → 影响传播 → 类型化门禁”的结果。固定点以 BranchRunState 为计算和放行边界；全局 RunState 只协调轮次并聚合分支结果。门禁发现可修复问题或人工输入改变证据时会产生稳定键工作项，对应分支尚未达到静止点，但不冻结无依赖的其他分支。
...
3. 对 `forward-ready` 子图按其适用链调用 active 分层物理设计单元；应用实体通常为 DWD → DWM → DM，显式 physical-model 实体截至 target_layer，依次产生通过 physical 门禁的 candidate。
4. 对完整适用 candidate 链执行应用端或目标物理模型契约验证，并渲染待确认模型 Markdown；验证失败产生补救 Need。
...
6. 对已确认子图计算 `round_delta`，传播状态、证据、模型阶段、策略和契约变化，并执行矩阵、粒度、循环、质量、安全、说明完整性和物理类型门禁。
...
10. 质量、安全、粒度、物理类型和方言门禁已通过，未产生新工作。
...
此时可以在应用查询中使用窗口聚合，但机构余额不得再沿产品维度求和。

### 7.3 强制规则

1. 改变粒度的普通聚合直接绑定共同物理提供方，不依赖另一个应用 Need。
2. 多个 Need 可以共享 DM，但必须保留独立原生粒度、口径和状态。
3. 报表查询对同一 DM 做 GROUP BY 或窗口计算，不产生新的 DM → DM 依赖。
...
9. `derivation_refs[]` 只用于多来源因果追溯，不参与拓扑排序；执行顺序使用依赖图的逆拓扑方向，从 DWD 向 DWM、DM 和应用端验证。
...
| 校验结果 | 行为 |
|---|---|
| pass | C-GATE/L-GATE 通过时推进到下一设计阶段；单层 P-GATE 通过时只登记 physical candidate；全部层 P-GATE 及完整候选链应用验证均通过后才生成待确认模型 Markdown；物理确认后的 DDL-GATE 通过时生成最终 Markdown 和对应物理节点的目标引擎 DDL |
...
| `security_classification` | string | 是 | 字段安全分级及访问限制 |
...
| SemanticKeyPartRole | `contract` | 标识类型、精度、SLA 或安全等契约差异 |
...
| WorkItemType | `reverse-solve` | 从应用 Need 向 DM、DWM、DWD 反向寻找能力缺口 |
...
| WorkItemType | `application-validation` | 验证完整候选链是否满足应用实体和 usage 契约 |
...
| GraphEdgeType | `binding-path` | 应用端与各层提供方之间的绑定跳 |
...
| EvidenceType | `policy-rule` | 依赖矩阵、命名、主题、质量、安全或层级策略规则 |
...
| GateType | `quality` | 校验质量规则、阈值、空值、唯一性和可观测性要求 |
| GateType | `security` | 校验分级、权限、脱敏、保留和审计要求 |
...
任何后续新增类型或枚举值必须先补充独立中文说明，再允许进入业务 CSV。`model_role`、`physical_type`、`security_classification` 等可扩展代码也不能作为无说明自由文本使用：每个实际值都必须先登记名称、说明、适用层/引擎和版本。
...
| `physical_fields.csv` | `physical_field_id`、`physical_model_id`、`logical_field_ref`、`column_name`、`business_name`、`description`、`physical_type`、`type_mapping_rule_ref`、`max_length`、`precision`、`scale`、`nullable`、`default_value`、`key_role`、`source_field_ref`、`transformation_rule`、`security_classification` | 解释字段物理类型、映射规则、长度、精度、标度、业务含义、来源、转换、键角色和安全限制；不适用的长度、精度或标度使用规范空值 |
...
1. `references/contracts/csv-schema.md`：逐一列出 3.7 节每个 CSV 的权威列集合、逻辑类型、必填性、空值、中文说明、主外键、唯一键、稳定排序键、枚举引用和安全分级。
...
1. 一个 Request 同时拆出报表、CSV 文件、指标和物理模型实体，所有对象可通过稳定 ID 追溯。
2. 全文和全部产物契约统一使用“应用端、应用实体、应用字段”术语。
3. 所有数仓领域结构化持久化和组件交换数据都位于已登记 CsvBundle 或 bootstrap 登记的 CSV 控制面；领域数据不存在 JSON、JSONL、YAML 或单元格内嵌数组/对象。宿主平台强制的最小 Skill 发现元数据予以豁免，但不得承载业务配置或运行状态。
4. 多值关系被拆成关联 CSV，manifest、行数、checksum、主外键、列序和稳定行序均可校验。
5. 每个记录类型、枚举值和 CSV 字段都有非空中文说明；缺少说明时门禁失败。
6. 报表和文件对同口径、同粒度字段共享 Need，但在 `entity_usages.csv` 中保留不同角色、必需性、展示粒度和验收规则。
7. 应用字段缺失时依次递归形成完整的 DM、DWM、DWD 规划链。
8. 每个 ProposedGraph 数仓模型节点都能追溯到 conceptual → logical → physical 三阶段版本链，任何阶段都不能跳过。
9. `reuse-as-is` 也必须验证三阶段证据；缺少证据时建立人工介入任务，不能仅凭物理表存在判定解决。
10. 应用实体反向按应用端 → DM → DWM → DWD 完成概念/逻辑规划，正向按 DWD → DWM → DM 完成物理设计；显式 physical-model 实体正向截至 target_layer。
11. DWD、DWM、DM 各自调用专属物理设计处理单元；reserved / not-configured 时物理确认和 DDL 均被阻止。
12. 每个物理确认记录绑定精确 physical fingerprint；conceptual、logical、physical、引擎或依赖快照变化会使下游阶段、确认和 DDL stale。
13. DDL 只读取 confirmed 物理模型；未指定引擎时使用 PostgreSQL，并生成表/字段 COMMENT。
14. 一个报表实体包含多个 grain Need，字段和指标分别绑定自己的原生粒度。
15. 产品余额与机构余额共享同一细粒度 DM，机构余额直接 GROUP BY 提供方，不依赖产品余额 Need。
16. 粗粒度指标广播到细粒度展示时标记不可沿新增维度求和；新增 grain-key 被识别为粒度变化。
17. 资产目录不完整时输出 candidate-new-unverified，不武断判定 create-new。
18. 非法边、Need 自依赖、同层绕路依赖和循环在写入时被拒绝；首次红线建立 HumanInterventionTask，不直接最终失败。
19. 缺少关键证据时相关 Need/usage 进入 awaiting-human，人工补充内容经校验和版本化后，以新 rerun_id 返回反向求解。
20. 人工不能直接绕过依赖矩阵；矩阵变化必须使用获授权且版本化的新规则或允许的例外证据。
21. 只有至少一次人工输入已 applied、受影响闭包完整复跑结束且同一 `failure_identity` 在 required 分支仍失败时，才产生 failed-after-intervention；新的失败 identity 重新建立首次人工任务。
22. 一个分支 awaiting-human 时，其他独立 `forward-ready` 分支仍可继续；只有自身物理确认及 DDL-GATE 通过后才能生成 DDL。
23. alternative 组按 `min_satisfied` 聚合；单个替代项失败不错误判定整个实体失败。
24. 子 Need 解决后全部父 Need 被重新入队并正确回卷；证据撤销或矩阵变化也能触发父级和下游 stale。
25. 派生指标通过 semantic dependency DAG 展开；标签展开规则输入，但普通聚合不产生语义依赖。
26. 门禁生成补救 Need 后继续迭代；只有 design_signature 连续两轮一致、三阶段与物理确认有效、DDL 资格门禁通过、DDL 生成验证完成且 delivery_signature 可复算，分支才宣布 converged。
27. awaiting-human、awaiting-physical-confirmation、awaiting-layer-implementation、nonconvergent 和 internal-failure 都不会被误报为成功或业务最终失败。
28. Bundle manifest 具有唯一根记录，SHA-256 bundle checksum 不包含 manifest 自身或根记录，能够稳定复算且不存在间接自引用。
29. 人工回流改变快照时保持同一 failure_identity、产生新的 occurrence；多个替代组逐组计算，只有全部相关 required 组满足时才通过，任一组满足也不能掩盖其他未满足组。
30. 新 Skill 开发前，3.7 节全部 CSV 均已在 schema reference 和 bootstrap 字典中逐字段定义；任一字段或类型缺少中文说明都会阻断实现。
31. 正常运行缺少资产库入口时登记不可查询的稳定空快照，完成实体和 Need 拆分后以 `cannot-determine` 建立可追溯的分支人工任务，不会在分支建立前生成无目标任务。
32. `execution_profile` 和资产目录 `source_kind` 进入快照及状态签名；production 模式在读取、Evidence、资产判定和最终门禁四处拒绝 test-fixture，且恢复执行不能改变已固定的来源身份。
33. 指定非默认引擎时只能使用 `engine_profile_metadata.csv` 声明相同 `engine_code` 的 Profile；缺少兼容 Profile 时进入 `awaiting-layer-implementation`，不会回退到 PostgreSQL 或生成错误方言 DDL。
